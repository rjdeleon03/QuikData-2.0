...
class EvacuationInfoViewModel @Inject constructor(private val mRepository: EvacuationInfoRepository)
    ...
    fun createEvacuationInfo(id: String) = runOnIoThread { mRepository.createData(id) }

    fun deleteEvacuationInfo(item: EvacuationItemDetails) = runOnIoThread {  mRepository.deleteData(item) }
}
