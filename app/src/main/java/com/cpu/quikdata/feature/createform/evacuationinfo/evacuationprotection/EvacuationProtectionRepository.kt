...
class EvacuationProtectionRepository @Inject constructor(
    private val mDatabase: AppDatabase,
    @EvacuationId evacuationId: String
) {

    private val mEvacuationProtection =
        mDatabase.evacuationProtectionDao().getByEvacuationId(evacuationId)

    val evacuationProtection: LiveData<EvacuationProtection>
        get() = mEvacuationProtection

    suspend fun updateData(data: EvacuationProtection) {
        mEvacuationProtection.value?.apply {
            copyFrom(data)
            mDatabase.evacuationProtectionDao().update(this)
        }
    }
}